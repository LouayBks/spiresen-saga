...
- `DOC/architecture/delivery_sequence.md` (WS-1..9, `ADR/ADR-008-work-slicing-and-sequencing.md`) — how the backlog is cut into full-stack feature PRs (infra → backend → frontend sub-branches), the current merge order, and the dev-environment push rule (WS-4): a push to any `dev/**` branch auto-applies its Terraform to the one shared dev environment, so never push another dev branch while one with unmerged infra is pushed unless it descends from it.
