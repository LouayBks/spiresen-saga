# S3 bucket (private, OAC-only access) + CloudFront distribution for a compiled SPA.
# domain_name/certificate_arn/route53_zone_id are optional — when unset, this serves
# off CloudFront's own default domain with no custom-domain wiring at all, which is
# what keeps this module reusable for an environment that isn't ready to pick a
# hostname yet.
...
resource "aws_cloudfront_distribution" "site" {
  ...
  # SPA client-side routing: CloudFront's S3-via-OAC origin returns 403 (not 404) for
  # a missing key, unlike the legacy public-website-endpoint origin — both codes must
  # redirect to index.html or any client-side route 403s on refresh.
