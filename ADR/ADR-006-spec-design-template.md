...
## Amendment (2026-09-18): a spec decides the domain model, never the physical storage shape

**This ADR decided requirement *format* (EARS/RFC 2119) and never explicitly bounded requirement *content* — that gap let two of the first four Stage-A specs drift into deciding literal DynamoDB `PK`/`SK` key patterns and item shapes, not just product behavior.** `naming.md` carried a full schema-rename table (`PK: USER#{userId} SK: SAGA#{sagaId}` → `PK: USER#{userId} SK: MAP#{mapId}`, etc.), and `alternate-views.md` invented the `Link`/`View` key patterns and reasoned directly about DynamoDB item-count tradeoffs (a `VIEWPOS#{viewId}#{nodeId}` item type vs. a single field). **Correction to this amendment's own first draft:** `box-sizing.md`'s `Node.tier` field and `content-authoring.md`'s `Node.images`/`coverImage`/`urls` field shapes are *not* the same mistake — naming a domain field that must exist is exactly what a spec is supposed to do; only `naming.md` and `alternate-views.md` actually crossed into physical key/item/transaction design. Both have since been amended in place (see each file's own "Amendment (2026-09-18, `ADR-006`)" note) to restate the same domain-model decisions as requirements for `#36` instead of as schema — this was corrected directly, not left as a flagged-only gap, per direct instruction.

**Why it's a mistake, on this project's own terms:** `spec_design_template.md`'s own stated principle is "a spec answers 'what', a plan answers 'how to build it safely'" — a DynamoDB key/item shape is *how something is persisted*, not *what the product does*, so it was never actually licensed by that principle even though two specs in a row did it anyway. It also directly conflicts with how ticket **#36** ("Data foundation — DynamoDB table + Saga/Box/Link/User CRUD") frames its own existence: sequenced *after* every Stage-A spec specifically "to avoid building the schema twice." A spec that already writes the schema doesn't just risk redundancy — it forecloses #36 from designing one coherent physical model across everything the specs decided, and it means every spec has to be revisited if the storage approach ever changes for reasons that have nothing to do with product behavior (e.g., moving off single-table design).

**The corrected rule, binding on every spec:** a spec may name entities, fields, and relationships that must exist ("a Map has a display name," "a Node has a `tier`"), and may state binding requirements the storage design must satisfy ("must be answerable in one query without a table scan," "must not vary per viewer," "must be atomic against retries") — but MUST NOT assert `PK`/`SK` literal patterns, DynamoDB item-type names, or transaction/write mechanics. Those belong exclusively to the data-foundation ticket (#36), which owns the single coherent schema realizing what every spec decided. `DOC/specs/accounts-and-auth.md` (#35) is the first spec drafted under this rule from scratch; `naming.md` and `alternate-views.md` are now amended to match it in place (their own "Amendment" notes mark exactly what changed); `box-sizing.md` and `content-authoring.md` needed no change — their field-level decisions were never the violation. Every one of them now follows the same pattern: state the property the storage must have, not the storage itself.

**Consequence for #36:** its own ticket text currently reads as pure implementation ("per the schema in `application_architecture.md`," as though the schema already exists) — under this amendment, #36's real scope includes the schema design itself for anything a spec stated as a requirement rather than a key pattern. That's a scope note for whoever picks up #36, not a rewrite of the ticket performed here.
