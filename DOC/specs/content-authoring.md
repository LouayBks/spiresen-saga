...
Format per `DOC/templates/spec_design_template.md` (ADR-006). Owns article/media content and the `note`/`urls` fields attachable to any box — orthogonal to `box-sizing.md` (`tier`) and `alternate-views.md` (position/links); no overlap, since content and its attachments don't vary per view or tier.
...
- Ticket: #33.
- Updates once accepted: `application_architecture.md`'s DynamoDB schema section (`ARTICLE#{id}`/`PRESENTATION#{id}`/`DESIGN#{id}` `DETAILS` records, plus new `Box.note`/`Box.urls` fields) and `boxes-plan.md` §7a's box-content-types section.
...
- **Notes and URLs are their own fields on `Box`, independent of `kind`.** Per the correction that a box "can contain other boxes, articles, videos, or images with notes, urls" — a note and a list of URLs can be attached to *any* box regardless of its `kind` (a `media` box can carry a note; a `group` box can carry a reference URL), not folded into the `note`-kind's own `body` field. `Box` gains `note?: string` and `urls?: [{url, label}]`, both optional, orthogonal to `kind` and `tier` the same way those two already are to each other.
...
|---|---|---|---|
| BHV-1 | WHEN the owner opens the editor for an `article`-kind box, THE editor SHALL present a markdown source pane and a live rendered preview pane side by side. | ✅ — component test typing markdown into the source pane and asserting the preview pane's rendered HTML updates | |
...
| BHV-4 | WHEN the owner attaches a note or a URL to any box, THE box SHALL persist it regardless of the box's `kind`. | ✅ — CRUD test attaching a note to a `media` box and a URL to a `group` box, asserting both persist | |
| BHV-5 | WHEN the owner saves an edit to an article's or media box's content, THE backend SHALL overwrite the existing `DETAILS` record with no retained prior version. | ✅ — integration test: edit twice, assert only the latest body is retrievable | |
...
| CON-5 | `Box.note` and `Box.urls` MUST be optional and MUST NOT be restricted to any particular `kind`. | |
