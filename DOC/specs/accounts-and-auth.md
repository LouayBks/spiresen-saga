...
- **Handle** — unique across every user, lowercase only: it MUST match `[a-z0-9_]+`, 3–20 characters. Platforms range from 15 (X) to 39 (GitHub), most commonly 30; 20 is the owner's choice, matching Reddit, Wattpad and DeviantArt, and keeps a handle short enough for a gallery row. 3 is the common floor (YouTube, Vimeo, Substack, Pinterest). Lowercase-only means uniqueness needs no case-folding. It is the user's public label wherever they are named (the public gallery, and any future profile or link).
- **Username** — free-text display name, not unique, 1–20 characters after trimming (the same ceiling as the handle). It may repeat across users and carries no identity or authorization meaning.
...
| CON-8 | A handle MUST match `[a-z0-9_]+` and be 3–20 characters; a username MUST be 1–20 characters after trimming. | Bounds decided 2026-09-19 by the owner, informed by platform norms (see Decisions). |
