...
| BHV-7 | WHEN the owner requests a new Map, THE backend SHALL create it with them as its sole owner and the default name `"My Map"`, independent of their existing Maps. | ✅ — integration test: an existing user requests a new Map, assert a second owned Map now exists, named `"My Map"`, with the first Map's data unchanged | |
