...
| BHV-6 | WHEN a Node is deleted, THE backend SHALL cascade-delete every Link referencing it in its own Map, and — if it has children — its entire nested Map (all its Nodes, Links and Views). | ✅ — integration test: delete a node with a nested Map containing further nodes/links, assert the whole nested Map is gone | |
...
| BHV-14 | WHEN the owner edits a Node's title, date or size, THE backend SHALL persist the change on any Node, whatever its current visualization. | ✅ — integration test: edit the title, date and size of an info node, an article node and a map node, assert each persists | Gap closed 2026-09-19 (`map-lifecycle.md` coverage matrix). Size rules: `box-sizing.md`. |
| BHV-15 | IF the owner triggers "Add inside" on a Node that holds a gallery or an article, THEN THE backend SHALL reject it as a conflict and create no nested Map. | ✅ — integration test: attach an article to a Node, attempt "Add inside", assert a 409 and that no nested Map exists; repeat with a gallery | Enforces `content-authoring.md` CON-8; the UI offers removing the gallery/article first. |
...
No implementation (that's #37/#38/#57). Moving a Node from one Map to another (re-parenting) is a deferred v2 feature, not designed here. Does not decide the exact visual design of the "+" affordance, "Add inside," the link-type picker, or the delete-midpoint control.
