...
- **Kind is chosen immediately at creation**, via a small inline picker (note / media / group) right after the "+" interaction — before size or content, since `kind` determines what the rest of the creation flow even looks like (a `media` node immediately offers an upload per `content-authoring.md`; a `group` node starts with zero children; a `note` node starts with empty title/body).
...
|---|---|---|---|
| BHV-1 | WHEN the owner clicks empty canvas space (or the toolbar "+"), THE canvas SHALL present an inline kind picker (note / media / group) before creating anything. | ✅ — component test: click empty canvas, assert the picker renders with exactly 3 options, no Node created yet | |
