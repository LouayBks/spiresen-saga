...
**Status:** proposed — `ADR/ADR-007-physical-data-layout.md` (decision, options weighed, gates, consequences) is the rationale, and `DOC/architecture/data_cartography.md` maps it entity by entity (access patterns, invariants, limits); both become authoritative when the owner merges PR #51. It supersedes the earlier box-tree sketch (`PK: BOX#… SK: BOX#/ARTICLE#/…`), which is no longer used.
