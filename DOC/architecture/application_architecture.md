...
**Correction (2026-09-18): `Box` does NOT get a stored `kind` attribute — superseded below, before `application_architecture.md`'s Map/Node/dynamic-visualization terminology gets fully propagated into this file.** A box's presentation (info / gallery / article / group) is inferred at read time from its own contents — 2+ child boxes under its own partition renders it as a group; a populated image gallery or an attached article `DETAILS` record renders it accordingly; neither renders it as a plain info box. No `kind` column is written or read. Size tier (S/M/L) is unaffected — still an explicit, stored, owner-chosen field, unrelated to this inference. **Video is not a hosted content type** — cost-driven, dropped entirely in favor of rich external links (e.g. YouTube, with a fetched-once thumbnail) — see `DOC/specs/content-authoring.md` for the full content-type list and CON-7/CON-8 for the inference order.
