...
**Status:** the layout below is proposed in `ADR/ADR-007-physical-data-layout.md` (decision, options weighed, consequences) and mapped entity by entity in `DOC/architecture/data_cartography.md` (access patterns, invariants, limits); it becomes authoritative on acceptance. It supersedes the earlier box-tree sketch (`PK: BOX#… SK: BOX#/ARTICLE#/…`), which is no longer used.
