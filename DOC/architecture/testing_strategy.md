...
| TS-4 | Pure, I/O-free business logic (id parsing and root-Map derivation, link-pair key canonicalization, handle/size/limit validation) gets small-tier `pytest` unit tests, no mocking | Agent/task def | Google's "small" test-size tier; the one place classic isolated unit testing is the right shape |
...
| TS-17 | Writer/reviewer separation is required for high-risk logic: any change touching auth/authorization checks, cross-slice authorization boundaries, or data-invariant transactions (`ADR-007`) gets its tests reviewed or written by a fresh-context or different-model pass before being considered complete | Agent/task def | Point 4 defines the concrete subagent/role split — this rule states *when* it's required, not *how* it's wired |
