...
| AP-9 | Delete a Node (cascade) | `TransactWriteItems`, then cleanup | One transaction: Delete the tile, Update META `nodeCount − 1` and, when nested, Update the parent tile `childCount − 1`. Then, best-effort: its `CONTENT#` items, its Links (`Query begins_with LINK#` on the Map, filter in code) and, if it has children, the whole nested partition (META, Views, Nodes) recursively. Not atomic above 100 items — see §6. Because reads verify the ancestor chain (AP-19), deleting the tile makes everything below it unreachable at once, even while cleanup lags. |
