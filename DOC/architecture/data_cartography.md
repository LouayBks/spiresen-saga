...
**Every `Update` below carries `attribute_exists(PK)`** — a bare update on a missing key would upsert — **except the deliberate upserts** (`DETAIL`: AP-13, AP-16, AP-17, each guarded by a check on its tile); it is not repeated on every row. **Reserved words:** attribute names such as `name`, `date`, `size`, `type` and `order` are DynamoDB reserved words, so every expression uses `ExpressionAttributeNames`.
...
| AP-12 | Rename a Map | `UpdateItem` on META, condition `attribute_exists(PK)` | The single canonical name (CON-6). |
...
| AP-21 | Change a Map's visibility | `UpdateItem` on the root META, condition `attribute_exists(PK)` | To `public`: set `visibility`, `publishedAt` and the index keys; leaving `public`: remove the last three. Owner only; rejects a nested Map (map-visibility BHV-2/3/12). |
| AP-22 | Change username | `UpdateItem` on the profile, condition `attribute_exists(PK)` | Single item, no uniqueness check (accounts-and-auth BHV-11). |
| AP-23 | Delete a Map (cascade) | `TransactWriteItems`, then cleanup | Owner-only. One transaction: Delete the root META (the Map is unreachable at once and leaves the gallery index in the same write) **and the owner's Membership** (v1 has exactly one; any other memberships, once sharing exists, are removed in the cleanup that follows). Then best-effort: remaining Nodes, Links, Views and content partitions (recursively into nested Maps) in bounded batches; not atomic above 100 items, leftovers are unreachable, not corrupt (`map-lifecycle.md` BHV-4/5). AP-2 skips — and lazily deletes — any Membership whose Map META is missing, so a lingering Membership never shows a deleted Map. |
...
| AP-25 | Rename / reorder / delete a View | `UpdateItem` / `TransactWriteItems` | Rename is a single conditional update; reorder rewrites the (at most three) Views' `order`; delete removes the View item first, then its Links and each tile's `positions[viewId]` best-effort, and closes the order gap (`map-lifecycle.md` BHV-8..10). |
| AP-26 | Add inside (create a nested Map) | `TransactWriteItems` | Put the nested META (`MAP#{nodeId}`, `attribute_not_exists`, `nodeCount = 0`, `viewCount = 1`), Put its default View (`linkCount = 0`, order 1), ConditionCheck that the Node's tile exists **and holds no gallery or article** (`imageCount = 0`, `hasArticle = false`; `content-authoring.md` CON-8 — the owner must remove those first). Idempotent: a repeat fails the condition and is treated as "already there". The nested META and Views **survive removal of the last child** (an empty nested Map; the Node is info again because `childCount = 0`) and are deleted only with the Node (AP-9) or the root Map (AP-23). |
...
| Map-load payload | ≲ 0.5 MB worst case, typically tens of KB | 50 tiles ≤ ~6 KB, ≤ 300 Links ≤ ~0.6 KB (label 100 chars), plus META and Views. **The ~6 KB tile assumes URL and key fields are ASCII** (`preview.url`, `thumbnailUrl`, `coverImage`: 1 byte per character); they are stored in their percent-encoded / punycode form and non-ASCII input is normalized or rejected (`content-authoring.md` CON-15). Counted at 4 bytes per character those three fields alone would be ~12 KB per tile and a worst-case Map load ~0.9 MB — the reason the rule exists. |
